...
import { Review, ReviewSlide, ReviewType } from '@/types/page/practicalTraining/TypePagePracticalTrainingPropsQuery'
import person from '@/public/assets/imgs/practicalCarousel/person.jpg'
            ...
            {review.map((el, idx) => 
              <SwiperSlide key={idx} className={stls.slide}>
                <PracticalReviewsCard
                  ...
                  name='Елена'
                  image={person}
                  answer={el.slide[idx].answer.replace(/\*\*(.*?)\*\*/g, '')
                    .replace(/^Ответ:\s*/, '')
                  .replace(/–/g, '\n–')
                  .trim()
                  }
