...
const StepBlocks = ({currentIndex}) => {
  ...
  return (
    <div className={stls.container}>
      {blocks.map((el) => 
        <div key={el.id} className={classNames(stls.stepBlock, { [stls.active]: filledBlocks[el.id] })}>
          <span 
          className={classNames(stls.number, { 
            [stls.completed]: filledBlocks[el.id], 
            })}>{el.id + 1}</span>
          <div className={stls.fill} style={{ height: `${filledBlocks[el.id] ? '100%' : '0%'}`, transition: 'height 0.8s linear' }} />
        </div>
      )}
    </div>
  )
