...
const checkLead = async (req, res) => {
  ...
  const {name, phone, email, utms, leadPage, referer, ymUid, access, utm, blockForAmo} = req.body
  console.log('inCHECKcreateLead', req.body)
  // res.status(400).json({ status: 400, msg: 'Lead created' })
  const data = {name, phone, email, price: null, leadId: '', promo: '', responsible_user_id: '', text: '', access, utm, ymUid, leadPage, blockForAmo}
