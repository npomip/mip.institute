...
const checkLead = async (req, res) => {
  ...
  try {
    ...
    if (checkPhoneResponse.status === 200) {
      ...
      const leadId =
        checkPhoneResponse.data._embedded.contacts[0]._embedded.leads[0].id
      const responsible_user_id =
        leadData._embedded.contacts[0].responsible_user_id
    ...
    console.error(
      'Ошибка при выполнении check or create lead:',
      error.response.data
    )
