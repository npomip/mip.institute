...
const checkLead = async (req, res) => {
  ...
  const {id, name, phone, email, utms, leadPage, referer, ymUid, access, utm, blockForAmo, promocode,edPartners} = req.body
  // console.log('inCHECKcreateLead', req.body)

  const data = {id, name, phone, email, price: null, leadId: '', promocode , responsible_user_id: '', text: '', access, utm, ymUid, leadPage, blockForAmo,edPartners}
